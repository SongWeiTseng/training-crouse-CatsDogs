...
def train_model(model, epochs, batch_size):
    total_data = [path for path in os.listdir('../train')]
    num_train = int(len(total_data)*0.8)
    train_data = total_data[:num_train]
    val_data = total_data[num_train:]
    train_set = data(train_data, True)
    val_set = data(val_data, False)
    ...
    for epoch in range(epochs):
        for phase in ['train', 'val']:
            ...
            if flag:
                print('epoch:{} training_loss:{} training_acc:{}'.format(epoch, str(total_loss / len(train_set)), str(acc / len(train_set))))
                ...
                if epoch_val_acc > best_acc:
                    best_acc = acc
                    best_model_wts = copy.deepcopy(model.state_dict())
                    torch.save(best_model_wts, '../weights/epoch%d_acc%.2f.pth' % (epoch, epoch_val_acc))
                print('epoch:{} val_loss:{} val_acc:{}'.format(epoch, str(total_loss / len(val_set)), str(epoch_val_acc)))
                ...
                with open('../log/val_acc.txt', 'a+') as file:
                    file.write('epoch_' + str(epoch + 1) + ' ' + str(acc / len(val_set)) + '\n')
...
if __name__ == '__main__':
    ...
    resnet = RestNet()
    train_model(resnet, epochs=100, batch_size=128)
